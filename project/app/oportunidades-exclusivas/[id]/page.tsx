// FILE: project/app/proyectos-exclusivos/[id]/page.tsx
...
function Lightbox(props: {
  ...
  const onTouchEnd = (e: React.TouchEvent<HTMLDivElement>) => {
    ...
    const dx = ch.clientX - touch.current.x0,
      dy = ch.clientY - touch.current.y0;
...
function getHeroImage(p?: Partial<Project> | null, fotos?: FotoRow[]) {
...
function HeroSealBadge({ tipo, tasa }: { tipo?: SealType | null; tasa?: number | null }) {
  if (!tipo) return null;

  const base =
    'absolute top-6 right-6 z-20 w-[100px] h-[100px] border border-white/70 bg-[#0A2E57]/85 text-white shadow-[0_16px_34px_rgba(0,0,0,0.30)] flex items-center justify-center';
    ...
    return (
      <div className={base}>
        <div className={inner}>
          ...
          <div className="text-[18px] font-semibold leading-none">{n != null ? `${n}%` : '—'}</div>
        </div>
      </div>
    );
  }
...
export default function ProyectoDetailPage({ params }: { params: { id: string } }) {
  ...
  const linea = [wordsCap(proj?.comuna), wordsCap(proj?.tipo), wordsCap(proj?.operacion)]
    .filter(Boolean)
    .join(' · ');
  ...
  return (
    <main className="bg-white">
      ...
      <section className="relative w-full overflow-hidden isolate">
        ...
        {/* SELLO EN HERO */}
        <HeroSealBadge tipo={proj?.sello_tipo ?? null} tasa={proj?.tasa_innovacion ?? null} />
            ...
            <div className="bg-white/70 backdrop-blur-sm shadow-xl p-4 md:p-5 w-full md:max-w-[480px]">
              ...
              <div className="mt-4 flex items-end gap-3">
                ...
                <div ref={priceBoxRef} className="ml-auto text-right">
                  ...
                  {precioClpHero && (
                    <div className="text-sm md:text-base text-slate-600 mt-[2px]">$ {nfCLP.format(precioClpHero)}</div>
                  )}
                </div>
              </div>
            </div>
...
function GalleryAndDetails({ proj, fotos }: { proj: Project | null; fotos: FotoRow[] }) {
