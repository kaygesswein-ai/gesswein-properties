...
const REGIONES: readonly Region[] = [
  'Arica y Parinacota',
  'Tarapacá',
  'Antofagasta',
  'Atacama',
...
const REG_NUM: Record<Region, number> = {
...
const BARRIOS: Record<string, string[]> = {
  ...
  'Ñuñoa': ['Plaza Ñuñoa', 'Villa Frei', 'Irarrázabal', 'Suárez Mujica'],
...
const toRoman = (n: number) => {
  ...
  let s = ''; let x = n;
  for (const [v, r] of pairs) while (x >= v) { s += r; x -= v; }
  return s;
};
const regionDisplay = (r: Region) => `${toRoman(REG_NUM[r])} - ${r}`;
...
export default function PropiedadesPage() {
  ...
  // Region: “X - Nombre” -> Nombre
  useEffect(() => {
    const m = regionInput.match(/^\s*[IVXLCDM]+\s*-\s*(.+)$/i);
    const name = (m ? m[1] : regionInput) as Region;
    setRegion((REGIONES as readonly string[]).includes(name) ? name : '');
  }, [regionInput]);

  // Construye query solo con lo relleno
  useEffect(() => {
    ...
    // Sólo disparamos el fetch (el backend ya debe aplicar estos params)
    if (trigger) fetch(`/api/propiedades?${p.toString()}`).catch(() => {});
  }, [
    trigger, qTop, operacion, tipo, region, comuna, barrio,
    minValor, maxValor, moneda, advancedMode,
    ...
    <main className="bg-white">
      ...
      >
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-14">
          <h1 className="text-3xl md:text-5xl font-light tracking-[0.35em] text-white">PROPIEDADES</h1>
          <p className="mt-2 text-white/90">Encuentra tu próxima inversión o tu nuevo hogar.</p>

          <div className="mt-6 relative">
            {/* placeholder vacío y padding izquierda reducido */}
            <input
              value={qTop}
