...
function getMapData(prop: Property | null) {
  if (!prop) {
    return {
      isReady: false,
      hasCoords: false,
  ...
  const lat = toFiniteNumber(prop.map_lat);
  const lng = toFiniteNumber(prop.map_lng);
  const zoomRaw = toFiniteNumber(prop.map_zoom);
...
export default function PropertyDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch(`/api/propiedades/${encodeURIComponent(params.id)}?ts=${Date.now()}`, {
          cache: 'no-store',
        });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch(`/api/propiedades/${encodeURIComponent(params.id)}/fotos?ts=${Date.now()}`, {
          cache: 'no-store',
        });
        const j = r.ok ? await r.json().catch(() => null) : null;
...
function GalleryAndDetails({ prop, fotos }: { prop: Property | null; fotos: FotoRow[] }) {
  ...
  const mapData = useMemo(() => getMapData(prop), [prop]);
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16">
        ...
        <div className="relative w-full h-[420px] border border-slate-200 overflow-hidden rounded bg-slate-100">
          {!mapData.isReady ? (
            ...
            <div className="absolute inset-0 flex items-center justify-center text-sm text-slate-500 text-center px-6">
              Esta propiedad no tiene coordenadas de mapa configuradas.
            </div>
          )}
        </div>
      </section>
